...
function LoginForm() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const redirectToUrl = `${window.location.origin}${targetPath.startsWith('/') ? targetPath : '/' + targetPath}`;
      ...
      <div className="w-full max-w-md space-y-8 relative z-10 animate-fade-in">
        ...
        <div className="bg-[var(--bg-base)] neu-raised p-8 rounded-3xl">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-[11px] font-bold text-[var(--text-main)] opacity-70 uppercase tracking-wider">
                  Password
                </label>
                <Link
                  href="/forgot-password"
                  className="text-xs font-semibold text-[#7C3AED] dark:text-[#8B5CF6] hover:underline"
                >
                  Forgot password?
                </Link>
              </div>
          ...
          <div className="mt-6 text-center pt-6 border-t border-[var(--shadow-dark)]/20">
            <p className="text-xs text-[var(--text-main)] opacity-80 font-semibold">
              Don&apos;t have an account?{' '}
              <Link
                href="/signup"
                className="font-extrabold text-[#7C3AED] dark:text-[#8B5CF6] hover:underline inline-flex items-center gap-1"
              >
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-[var(--bg-base)]">Loading...</div>}>
      <LoginForm />
    </Suspense>
  );
}
